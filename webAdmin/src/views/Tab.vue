...
import addModal from '@/components/tab/add-modal'
import editModal from '@/components/tab/edit-modal'
import detailModal from '@/components/tab/detail-modal'
...
export default {
  ...
  data () {
    return {
      ...
      total: 0,
      searchKeyList: ['id', 'name', 'zh', 'sort_num'],
      query: {
        page: 1,
        page_size: 10,
        odb_: null,
  ...
  methods: {
    ...
    async getData () {
      let res = await tabAPI.list(Object.assign({}, this.query))
    ...
    async handleDelete (row) {
      this.$Modal.confirm({
        title: '警告!',
        content: `确定要删除${row.name} tab吗?`,
        okText: '确定',
        cancelText: '取消',
        onOk: async () => {
          try {
            await tabAPI._delete({ 'uid': row.uid })
